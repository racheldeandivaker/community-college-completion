...
Importing and cleaning data from directory and program completions using CIP_6 codes. 2022 is the last available year. 
...
#cipcode_6digit: above 0 - specific program name

clean_comp <- comp_data %>%
  select(unitid, cipcode_6digit, award_level, awards_6digit, fips, year) %>%
  filter(fips == 12, cipcode_6digit > 0, award_level %in% c(1, 2, 4, 30, 31, 32, 33), awards_6digit > 0)
...
##ask Zyra- I think I need to filter out 99? I believe 99 is all programs combined? 

```

Get top five Certificate programs for each college
```{r}
# Filter to Certificate programs (award_levels 1, 2, 30, 31, 32, 33)
certificates <- merged_data %>%
  filter(award_level %in% c(1, 2, 30, 31, 32, 33))

# Sum actual awards per college + program
