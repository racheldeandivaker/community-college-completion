...
- Save results to `data/fl_cc_programs.csv`

### Output

Both scripts save the analysis results to a CSV file:
- **Output file**: `data/fl_cc_programs.csv`
- **R Markdown**: Also generates formatted PDF reports for documentation
- **R Script**: Provides console output showing progress and confirmation of saved results

The output file (`fl_cc_programs.csv`) contains:
...
After running either script, you can also directly inspect the `merged_data` object in your R environment:
```r
...
│   └── fl_cc_programs.csv         # Analysis output (generated by scripts)
